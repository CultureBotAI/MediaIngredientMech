# `data/ingredients/mapped/2-Chlorobenzoic_acid.yaml`

## Verdict

Needs curation, minor. The CultureMech residual grounding to `CHEBI:30793` is
...
- Reviewed record: `data/ingredients/mapped/2-Chlorobenzoic_acid.yaml`.
- Identifier and grounding: `identifier: CHEBI:30793` with
  `ontology_mapping.ontology_id: CHEBI:30793`, source `CHEBI`,
  `mapping_quality: EXACT_MATCH`, `match_level: EXACT`, and
  `mapping_status: MAPPED`.
- Official ChEBI check: the current EMBL-EBI ChEBI page for `CHEBI:30793`
  resolves to `2-chlorobenzoic acid` and lists CAS `118-91-2`, formula
  `C7H5ClO2`, and InChI
...
- `uv run --frozen linkml-term-validator validate-data data/ingredients/mapped/2-Chlorobenzoic_acid.yaml -s src/mediaingredientmech/schema/mediaingredientmech.yaml -t IngredientRecord --labels`:
  passed.
- `scripts/_engine_a_obo_safe.sh data/ingredients/mapped/2-Chlorobenzoic_acid.yaml "CHEBI FOODON NCIT MESH UBERON ENVO BTO PATO"`:
  passed.
...
- `mappings/ingredient_mappings.sssom.tsv` contains the expected exact
  `MIM:2-Chlorobenzoic_acid` to `CHEBI:30793` row with the restored
...
- The hidden/ignored-inclusive search over YAML, TSV, Markdown, ignored
  ...
  rows and no unresolved active duplicate for `CHEBI:30793`.
...
1. In `data/ingredients/mapped/2-Chlorobenzoic_acid.yaml`, add

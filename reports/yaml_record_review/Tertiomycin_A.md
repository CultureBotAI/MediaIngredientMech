# `data/ingredients/mapped/Tertiomycin_A.yaml`

## Verdict

Needs curation - major. The kg-microbe placeholder identity is still the best
available local fallback for `Tertiomycin A`, but its `SELECTIVE_AGENT` role is
only a provisional computational name-pattern prediction.

## Identity

- Reviewed record: `data/ingredients/mapped/Tertiomycin_A.yaml`.
- Identifier and grounding:
  `identifier: kgmicrobe.compound:tertiomycin_a` with the same
  `ontology_mapping.ontology_id`, label `Tertiomycin A`, source
...
- Fresh exact OLS4 search for `Tertiomycin A` returned zero results, and fresh
...
- `mappings/ingredient_mappings_unknown_term_manual_candidate_review.tsv`
  ...
  matches for Tertiomycin A, and
  `mappings/ingredient_mappings_unknown_term_placeholder_ols_candidates.tsv`
  records no exact 2026-05-06 candidate across CHEBI, MeSH, NCIT, MICRO, BTO,
  and FOODON.
- The final SSSOM has exactly one exact local registry row for
  `MIM:Tertiomycin_A`, points at `kgmicrobe.compound:tertiomycin_a`, names
...
- Major: replace the provisional name-pattern `SELECTIVE_AGENT` role in
  `data/ingredients/mapped/Tertiomycin_A.yaml` with source-backed evidence for
  Tertiomycin A as a selective agent, or remove the role if no maintained
  source supports it.

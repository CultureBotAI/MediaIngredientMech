# `data/ingredients/mapped/Nicl2_X_6_H2o.yaml`

## Verdict

Pass with minor issues. This duplicate hexahydrate record has been tombstoned
as `REJECTED` and merged into the active `CHEBI:53542` record, so it no longer
publishes final SSSOM rows; stale role, synonym, and structure fields remain
only on the rejected tombstone.

## Identity

- Reviewed record: `data/ingredients/mapped/Nicl2_X_6_H2o.yaml`.
- Identifier and grounding: `identifier: CHEBI:53542` with
  ...
  `mapping_quality: EXACT_MATCH`, `mapping_status: REJECTED`,
  `kg_microbe_node_id: CHEBI:53542`, and
  `ingredient_type: SINGLE_INGREDIENT`.
- Occurrences: 0 active occurrences; the previous hexahydrate occurrences were
  transferred to `Nickel_II_chloride_hexahydrate.yaml`.
...
- A fresh EBI OLS4 lookup resolves `CHEBI:53542` as active
  `nickel chloride hexahydrate` with formula `2Cl.6H2O.Ni` and CAS
  `7791-20-0`.
- The August 2026 merge history records that this row was merged into
  `CHEBI:53542` `Nickel (II) chloride hexahydrate`; the later tombstone
  repairs synchronized `ontology_id` and `kg_microbe_node_id` to that same
  target.
- Filtering the final `mappings/ingredient_mappings.sssom.tsv` produced no
  active `MIM:Nicl2_X_6_H2o` row, which is correct for the tombstone.
...
- Minor: the migrated `TRACE_ELEMENT` role remains on the rejected tombstone.
...
- Minor: in `data/ingredients/mapped/Nicl2_X_6_H2o.yaml`, clear or refresh the

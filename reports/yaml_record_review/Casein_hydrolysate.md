# `data/ingredients/mapped/Casein_hydrolysate.yaml`

## Verdict

Needs curation; major issue. The exact `MICRO:0001366` casein-hydrolysate
grounding, acid-hydrolysate synonym decision, occurrence count, SSSOM row, and
aggregate copy agree, but `PROTEIN_SOURCE` is asserted from only a provisional
name-pattern prediction.

## Identity

- Reviewed record: `data/ingredients/mapped/Casein_hydrolysate.yaml`.
- Identifier and grounding: `identifier: MICRO:0001366`,
  `ontology_mapping.ontology_id: MICRO:0001366`,
  `ontology_label: casein hydrolysate`, `ontology_source: MICRO`,
  `mapping_quality: EXACT_MATCH`, `mapping_status: MAPPED`, and
  `ingredient_type: UNDEFINED_MIXTURE`.
- Direct OLS lookup for `MICRO:0001366` returns one active MICRO term labelled
  `casein hydrolysate`.
...
- Hidden/ignored-inclusive search over `data/curated`, `mappings`,
  ...
  backups, found the active `MIM:Casein_hydrolysate` SSSOM row with
  `MICRO:0001366`, the prefix-specific OLS validation row, and matching
  aggregate/docs rows.
- The `Acid hydrolysate of casein` synonym is backed by the #260 curation note
  that explicitly reviewed the MICRO term's lack of an acid/enzymatic split.
- The current `mappings/culturemech_recipe_membership.tsv` rows contain 54
  distinct recipes and 54 total occurrences for `MICRO:0001366`, matching
...
- The MICRO identifier, acid-hydrolysate CultureMech gap label, 54/54
...
- Major: either replace the provisional
  ...
  `data/ingredients/mapped/Casein_hydrolysate.yaml` with inspected evidence for
  this casein-hydrolysate scope, or remove the role, then rerun strict

# `data/ingredients/mapped/Bacto_Tryptic_Soy_Agar_Difco.yaml`

## Verdict

Needs curation, minor. The record is correctly tombstoned as a duplicate of the
live `Bacto Tryptic Soy Agar` record at `MICRO:0000114`, but its top-level
...
- Prefix-specific OLS exact search in `micro` resolves `MICRO:0000114` to
  `tryptic soy agar`.
- The 2026-08-05 `MERGED_INTO` event says this `(Difco)` spelling was merged
  into the live `MICRO:0000114` `Bacto Tryptic Soy Agar` record; no separate
...
- Hidden/ignored-inclusive search over `data/curated`, `mappings`,
  ...
  rejected tombstone.
- The current raw synonym `Bacto Tryptic Soy Agar (Difco)` is the original
...
- `mappings/ingredient_mappings_external_prefix_ols_validation.tsv` and the
  fresh OLS exact query both resolve `MICRO:0000114` to `tryptic soy agar`, so
...
- Minor: update `notes` in
  `data/ingredients/mapped/Bacto_Tryptic_Soy_Agar_Difco.yaml` to describe the
  current rejected tombstone state and the live `Bacto Tryptic Soy Agar`

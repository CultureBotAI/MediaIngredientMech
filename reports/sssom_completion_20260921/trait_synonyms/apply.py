"""Apply only the frozen, reviewed non-name synonym decisions for MIM #703.

The default invocation validates the plan without writing ingredient records.
Use --apply for the guarded write, or --verify to require the completed state.
...
def yaml_bytes(record: dict) -> bytes:
    return yaml.safe_dump(
        record, default_flow_style=False, sort_keys=False, allow_unicode=True, width=80
    ).encode()
...
def reviewed_after(entry: dict, timestamp: str) -> dict:
    ...
    record_curation_event(
        after,
        curator="claude",
        action="REJECTED_TRAIT_SYNONYMS",
        changes=(
            "Retyped activity/assay phrases as provenance-only REJECTED_LABEL "
            "because they describe a process or organism trait, not an ingredient "
            "name (#703): "
            + "; ".join(repr(token) for token in tokens)
            + ". Preserved original text and source metadata. No identity, "
            "mapping, role, or remaining-synonym claim was adjudicated."
        ),
        llm_assisted=True,
        timestamp=timestamp,
    )
...
def validate_plan(plan: dict, root: Path) -> list[tuple[Path, dict, bool]]:
    if plan["issue"] != ISSUE or plan["schema_version"] != 1:
        raise ValueError("Unexpected review plan")
    entries = plan["records"]
    if len(entries) != 39 or sum(len(e["decisions"]) for e in entries) != 46:
        raise ValueError("The reviewed scope is exactly 39 records / 46 tokens")
    paths = set()
    prepared = []
    for entry in entries:
        ...
        if not done and (
            current != entry["before_record"]
            or sha256_bytes(current_bytes) != entry["before_yaml_sha256"]
        ):
...
def main() -> None:
    ...
    prepared = validate_plan(plan, args.root)
    ...
    if args.apply and pending:
        ...
        # All source records and their final payloads were checked before any write.
    ...
    print(
        json.dumps(
            {
                "status": "PASS",
                "records_validated_before_and_after": len(prepared),
                "trait_tokens_reviewed": 46,

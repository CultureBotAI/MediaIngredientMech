"""Apply only the reviewed #703 synonym delta, checked by the pinned producer.
...
def main():
    ...
    plan = json.loads((HERE / "trait_synonyms/decisions.json").read_text())
    target = ROOT / plan["baseline_sssom"]["path"]
    original = target.read_bytes()
    if digest(original) != plan["baseline_sssom"]["sha256"]:
        raise ValueError("Source differs from the reviewed #703 baseline")
    ...
    for entry in plan["records"]:
        ...
        for reviewed in entry["sssom_rows"]:
            ...
            if row != old:
                raise ValueError("Source row differs from reviewed trait decision")
            ...
            if rejected & published:
                raise ValueError("Pinned producer republished a rejected trait")
    ...
    (HERE / "trait-synonym-refresh.json").write_text(json.dumps(receipt, indent=2) + "\n")

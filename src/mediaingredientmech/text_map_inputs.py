...
import yaml  # type: ignore[import-untyped]  # PyYAML is untyped in the locked environment.

REPO_ROOT = Path(__file__).resolve().parents[2]
ADAPTER_VERSION = "mediaingredientmech-semantic-v1"
RECORD_ROOTS = ("data/ingredients/mapped", "data/ingredients/unmapped")
...
def include_record(record: dict) -> bool:
    return record.get("mapping_status") != "REJECTED"
...
def record_details(record: dict, path: Path) -> tuple[str, str, str, str]:
    ...
    return (
        ...
        clean(record.get("preferred_term")),
        clean(record.get("ingredient_type")) or "UNKNOWN",
        "browser.html#record=" + quote(source, safe=""),
    )
...
def semantic_text(record: dict, context: dict | None = None) -> str:
    ...
    lines: list[str] = []
    add(lines, "name", record.get("preferred_term"))
    add(lines, "ingredient type", enum_text(record.get("ingredient_type")))
    synonyms = sorted(
        {
            clean(item.get("synonym_text"))
            for item in record.get("synonyms") or []
            if is_resolving_synonym(item)
        }
        - {""}
    )
    add(lines, "synonyms", "; ".join(synonyms))
    for field in ("description", "definition"):
        add(lines, field, record.get(field))
    properties = record.get("chemical_properties") or {}
    for field in ("molecular_formula", "molecular_weight", "solubility", "physical_state"):
        add(lines, field.replace("_", " "), properties.get(field))
    for component in record.get("components") or []:
        add(lines, "component", label_of(component))
        for field in ("concentration", "proportion", "role"):
            value = component.get(field)
            if isinstance(value, (str, int, float)):
                add(lines, field, value)
    return "\n".join(lines) + "\n"

...
def test_real_curated_corpus_has_no_partonomy_violations():
    ...
    # #710 withdrew the four-part CMC/PY/serum blend after the original
    # source showed separate culture contexts, not one ingredient mixture.
    assert len(parents) == 83
    assert sum(len(record["components"]) for record in parents) == 502

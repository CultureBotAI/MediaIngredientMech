...
def test_bare_external_role_evidence_still_lacks_external_source(reference_type):
    ...
    assert "role_lacks_external_evidence:nutritional_roles.CARBON_SOURCE" in row.issues
    assert (
        f"external_role_evidence_missing_locator:nutritional_roles.CARBON_SOURCE.{reference_type}"
        in row.issues
    )
...
def test_blank_external_role_locator_still_lacks_external_source():
    ...
    assert "role_lacks_external_evidence:nutritional_roles.CARBON_SOURCE" in row.issues
    assert (
        "external_role_evidence_missing_locator:nutritional_roles.CARBON_SOURCE.DATABASE_ENTRY"
        in row.issues
    )


def test_blank_role_citation_fields_do_not_increase_evidence_score():
    assert (
        _score.citation_score(
            {
                "reference_type": "COMPUTATIONAL_PREDICTION",
                "doi": "  ",
                "pmid": "  ",
        ...
        == _score.CITATION_TYPE_POINTS["COMPUTATIONAL_PREDICTION"]
    )


def test_located_and_bare_external_role_evidence_reports_bare_citation():
    record = _record(
        nutritional_roles=[
            {
                "role": "CARBON_SOURCE",
                ...
                "evidence": [
                    {
                        ...
                        "reference_text": "ChEBI role annotation",
                    },
    ...
    row = _score.score_record(Path("mixed.yaml"), record)

    assert "role_lacks_external_evidence:nutritional_roles.CARBON_SOURCE" not in row.issues
    assert (
        "external_role_evidence_missing_locator:nutritional_roles.CARBON_SOURCE.TECHNICAL_REPORT"
        in row.issues
    )
...
def test_located_external_role_evidence_clears_external_source_issue(reference_type, locator):
    ...
    assert "role_lacks_external_evidence:nutritional_roles.CARBON_SOURCE" not in row.issues
    assert not any(
        issue.startswith("external_role_evidence_missing_locator:") for issue in row.issues
    )

"""The oaklib pin must keep `sqlite:obo:` off the retiring semantic-sql S3 bucket (#488).
...
it; 0.7.2 moved the default to the CDN. #493 did the migration and nothing has
guarded it since.

The reversion is invisible until it is catastrophic. On 0.6.23 every ChEBI
lookup still succeeds today, so no test, gate or error message distinguishes a
correct pin from a reverted one — right up until the bucket goes away and every
`get_adapter("sqlite:obo:...")` call fails at once. That includes the
`label-correspondence` gate, which downloads through this path on a cache miss.
...
# lowered to 0.7.2 and quietly undo the alignment — the defect TraitMech#572
# describes in its equivalent guard.
MIN_OAKLIB = (0, 7, 3)

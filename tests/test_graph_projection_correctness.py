...
def write_record(root, stem, identifier, category="mapped", **extra):
    ...
    path.write_text(yaml.safe_dump({"identifier": identifier, "preferred_term": stem,
                                   "mapping_status": category.upper(), **extra}))
...
def test_missing_mapped_and_unmapped_are_omitted_with_coverage(tmp_path, monkeypatch):
    ...
    vectors = {"CHEBI:1": np.array([1., 2., 3.]), "CHEBI:2": np.array([4., 5., 6.])}
    ...
    assert frame.attrs["coverage"] == {"eligible_records": 4, "embedded_records": 2,
        "missing_records": ["MIM:missing_mapped", "MIM:missing_unmapped"], "synthetic_records": 0}
...
def test_actual_lookup_method_is_retained(record, expected):
    generator = IngredientUMAPGenerator({"CHEBI:1": np.ones(3), "mediadive.ingredient:2": np.ones(3)})
